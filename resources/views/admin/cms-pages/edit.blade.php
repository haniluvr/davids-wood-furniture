...
<div class="min-h-screen bg-white">
    ...
    <div class="bg-white shadow-sm border-b border-stone-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-6">
                <div>
                    <h1 class="text-2xl font-bold text-stone-900">Edit CMS Page</h1>
                    <p class="mt-1 text-sm text-stone-600">Edit "{{ $cmsPage->title }}"</p>
                </div>
                <div class="flex gap-3">
                    <a href="{{ admin_route('cms-pages.preview', $cmsPage) }}" target="_blank" class="inline-flex items-center px-4 py-2 border border-stone-300 rounded-lg text-sm font-medium text-stone-700 bg-white hover:bg-stone-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <form action="{{ admin_route('cms-pages.update', $cmsPage) }}" method="POST" class="space-y-6" id="cmsForm">
        @csrf
        @method('PUT')
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        required
                                    >
                                        <option value="page" {{ old('type', $cmsPage->type) === 'page' ? 'selected' : '' }}>Page</option>
                                        <option value="blog" {{ old('type', $cmsPage->type) === 'blog' ? 'selected' : '' }}>Blog Post</option>
                                        <option value="faq" {{ old('type', $cmsPage->type) === 'faq' ? 'selected' : '' }}>FAQ</option>
                                        <option value="policy" {{ old('type', $cmsPage->type) === 'policy' ? 'selected' : '' }}>Policy</option>
                                    </select>
                                    @error('type')
                                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                    @enderror
                ...
                <div class="lg:col-span-2">
                        ...
                        Featured Image
                                    ...
                                    @if($cmsPage->featured_image)
                                        <div class="mb-4 p-4 border border-stone-300 rounded-lg bg-stone-50">
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center space-x-3">
                                                    <img src="{{ $cmsPage->featured_image_url }}" alt="{{ $cmsPage->title }}" class="w-16 h-16 object-cover rounded">
                                                    <div>
                                                        <p class="text-sm font-medium text-stone-900">Current Featured Image</p>
